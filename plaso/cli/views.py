...
class CLITableView(BaseTableView):
  ...
  def _WriteRow(self, output_writer, values):
    ...
    secondary_format_string = f'{{0:<{self._column_width + 3:d}s}}{{1:s}}\n'

    if isinstance(values[1], str):
      value_string = values[1]
    else:
      value_string = f'{values[1]!s}'
  ...
  def Write(self, output_writer):
    ...
    if self._number_of_columns not in (0, 2):
      raise RuntimeError(f'Unsupported number of columns: {self._number_of_columns:d}.')
...
class CLITabularTableView(BaseTableView):
  ...
  def _WriteRow(self, output_writer, values, in_bold=False):
    ...
    if in_bold and not win32console:
      ...
      row_strings = f'[1m{row_strings:s}[0m'

    row_strings = f'{row_strings:s}\n'
...
class MarkdownTableView(BaseTableView):
  ...
  def _WriteHeader(self, output_writer):
    ...
    """
    if self._title:
      output_writer.Write(f"{'#' * self._title_level:s} {self._title:s}\n\n")
  ...
  def _WriteRow(self, output_writer, values):
    ...
    """
    if self._columns:
      row_values = [f'{value!s}' for value in values]
      output_writer.Write(' | '.join(row_values))
      output_writer.Write('\n')
    else:
      row_values = ''.join([
         f'<td>{value!s}</td>' for value in values[1:]])
      output_writer.Write((
