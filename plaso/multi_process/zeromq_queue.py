...
class ZeroMQQueue(plaso_queue.Queue):
  ...
  def _SendItem(self, zmq_socket, item, block=True):
    ...
    """
    try:
      logger.debug(f'{self.name:s} sending item')
      if block:
        zmq_socket.send_pyobj(item)
      else:
        zmq_socket.send_pyobj(item, zmq.DONTWAIT)
      logger.debug(f'{self.name:s} sent item')
      return True

    except zmq.error.Again:
      logger.debug(f'{self.name:s} could not send an item')

    except zmq.error.ZMQError as exception:
      if exception.errno == errno.EINTR:
        logger.error(
            f'ZMQ syscall interrupted in {self.name:s}.')
  ...
  def _ReceiveItemOnActivity(self, zmq_socket):
    ...
    if events:
      try:
        ...
        logger.error(
            f'{self.name:s}. Failed to receive item in time.')
        raise

      except zmq.error.ZMQError as exception:
        if exception.errno == errno.EINTR:
          logger.error(
              f'ZMQ syscall interrupted in {self.name:s}. Queue aborting.')
        raise
  ...
  def _CreateZMQSocket(self):
    ...
    logger.debug(f'Creating socket for {self.name:s}')
    ...
    if self._zmq_socket:
      logger.debug(f'Closing old socket for {self.name:s}')
    ...
    if self.port:
      address = f'{self._SOCKET_ADDRESS:s}:{self.port:d}'
      if self.SOCKET_CONNECTION_TYPE == self.SOCKET_CONNECTION_CONNECT:
        self._zmq_socket.connect(address)
        logger.debug(f'{self.name:s} connected to {address:s}')
      else:
        self._zmq_socket.bind(address)
        logger.debug(
            f'{self.name:s} bound to specified port {address:s}')
    else:
      self.port = self._zmq_socket.bind_to_random_port(self._SOCKET_ADDRESS)
      logger.debug(
          f'{self.name:s} bound to random port {self.port:d}')
  ...
  def Close(self, abort=False):
    ...
    if abort:
      if not self._closed_event.is_set():
        logger.warning(
            f'{self.name:s} queue aborting. Contents may be lost.')
...
class ZeroMQPullQueue(ZeroMQQueue):
  ...
  def PopItem(self):
    ...
    logger.debug(
        f'Pop on {self.name:s} queue, port {self.port:d}')
...
class ZeroMQPushQueue(ZeroMQQueue):
  ...
  def PushItem(self, item, block=True):
    ...
    logger.debug(
        f'Push on {self.name:s} queue, port {self.port:d}')
    ...
    while not self._terminate_event.is_set():
      try:
        ...
        if time.time() > last_retry_timestamp:
          logger.error(f'{self.name:s} unable to push item, raising.')
...
class ZeroMQRequestQueue(ZeroMQQueue):
  ...
  def PopItem(self):
    ...
    logger.debug(f'Pop on {self.name:s} queue, port {self.port:d}')
    ...
    while not self._terminate_event.is_set():
      try:
        ...
        if time.time() > last_retry_time:
          logger.warning(f'{self.name:s} timeout requesting item')
          raise errors.QueueEmpty
...
class ZeroMQBufferedQueue(ZeroMQQueue):
  ...
  def Close(self, abort=False):
    ...
    if abort:
      ...
      if self._zmq_thread:
        logger.debug(f'[{self.name:s}] Waiting for thread to exit.')
...
class ZeroMQBufferedReplyQueue(ZeroMQBufferedQueue):
  ...
  def _ZeroMQResponder(self, source_queue):
    ...
    logger.debug(f'{self.name:s} responder thread started')
    ...
    while not self._terminate_event.is_set():
      ...
      if not sent_successfully:
        logger.error(f'Queue {self.name:s} unable to send item.')
        break

    logger.info(f'Queue {self.name:s} responder exiting.')
