...
class RedisMergeReader(interface.StorageMergeReader):
  ...
  def _GetAttributeContainers(
    ...
    for identifier_bytes, serialized_container in iter(items.items()):
      ...
      self._store.RemoveAttributeContainer(container_type, identifier)
  ...
  def MergeAttributeContainers(self, callback=None,
    ...
    while (self._active_cursor or len(self._container_types) != 0 or len(self._active_extra_containers) != 0):
      ...
      if (maximum_number_of_containers > 0 and
        ...
        logger.debug('Only merged {0:d} containers'.format(number_of_containers))
        return False
